	...
	namespace Hook
	{
		...
		bool WINAPI Hook_CreateMove(float flInputSampleTime, CUserCmd* pCmd)
		{
			...
			if (Interfaces::Engine()->IsConnected() || Interfaces::Engine()->IsInGame())
			{
					...
					{
						if (GetAsyncKeyState(VK_LBUTTON) || HasGrenade || GetAsyncKeyState(VK_RBUTTON) || GetAsyncKeyState(VK_MENU) || pCmd->buttons & IN_GRENADE1 || pCmd->buttons & IN_GRENADE2 || pCmd->buttons & IN_ATTACK || pCmd->buttons & IN_ATTACK2)
						{
							Settings::Misc::misc_LegitAA = false;
						}
						else if (!GetAsyncKeyState(VK_LBUTTON) || HasGrenade || !GetAsyncKeyState(VK_RBUTTON) || !GetAsyncKeyState(VK_MENU) || !pCmd->buttons & IN_GRENADE1 || !pCmd->buttons & IN_GRENADE2 || !pCmd->buttons & IN_ATTACK || !pCmd->buttons & IN_ATTACK2)
						{
				...
				if (Settings::Misc::misc_LegitAA && !(pCmd->buttons & IN_ATTACK) && !(pCmd->buttons & IN_GRENADE1) && !(pCmd->buttons & IN_GRENADE2) && !(pCmd->buttons & IN_ATTACK2) && (pCmd->tick_count % 2) && pLocal->GetMoveType() != MOVETYPE_LADDER && pLocal->GetMoveType() != MOVETYPE_NOCLIP)
				{
					...
					//a d
					if (GetKeyState(0x41) & 0x80) dir = true;
					if (GetKeyState(0x44) & 0x80) dir = false;
